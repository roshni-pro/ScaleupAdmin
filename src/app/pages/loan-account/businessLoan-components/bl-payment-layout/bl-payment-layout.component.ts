...
@Component({
  selector: 'app-bl-payment-layout',
  templateUrl: './bl-payment-layout.component.html',
  styleUrls: ['./bl-payment-layout.component.scss']
})
export class BLPaymentLayoutComponent {
  ...
  paymentStatusList = [
    ...
    { value: 'Partially Paid', label: 'Partial Paid' },
  ...
  getAccountTransaction(loadType: any) {
    ...
    this.Loader = true;
    this.loanService.GetAccountTransaction(this.searchPayload).subscribe((x: any) => {
  ...
  GetAccountTransactionExport() {
    ...
    this.loanService.GetAccountTransactionExport(this.searchPayload).subscribe((x: any) => {
      console.log("transactionList", x);
      this.Loader = false;
      if (x && x.result.length > 0) {
  ...
  onexport(expData: any) {
    ...
    expData.forEach((element: any) => {
      element.transactionDate = element.transactionDate == "1900-01-01T00:00:00" ? '-' : element.transactionDate;
