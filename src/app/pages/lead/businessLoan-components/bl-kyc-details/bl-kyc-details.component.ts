...
@Component({
  selector: 'app-bl-kyc-details',
  templateUrl: './bl-kyc-details.component.html',
  styleUrls: ['./bl-kyc-details.component.scss']
})
export class BlKycDetailsComponent implements AfterViewInit {
    ...
    async GetLeadDetails() {
        if (this.userId != null) {
            ...
            if (res) {
                ...
                this.router.navigate(['pages/lead/BL-leads/Bl-Kyc-Detail/' +
...
// && (this.customerData?.leadGenerator?true:false) && (this.customerData?.leadConvertor?true:false)
    ...
    OnBack() {
        this.router.navigateByUrl('pages/lead/BL-leads');
    }
