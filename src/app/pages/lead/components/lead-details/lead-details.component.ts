...
export class LeadDetailsComponent implements OnDestroy {
    ...
    addLeadGeneratorConvertor() {
        ...
        if (this.isValidate) {
            ...
            this.loader = true;
            this._leadService.AddLeadGeneratorConvertor(payload).subscribe((res:any) => {
                console.log(res);
                if (res) {
                    this.loader = false;
                ...
                accept: () => {
                    try {
                        ...
                        // this._leadService.InitiateLeadOffer(this.leadId).subscribe(res => {
                            this._leadService.InitiateLeadOffer(payload).subscribe((res:any) => {
                                if (res.status) {
                                this.loader = false;
    ...
    reset() {
        if (this.leadId > 0) {
            this.confirmationService.confirm({
                ...
                accept: () => {
                    this.loader = true;
                    this._leadService.ResetLead(this.leadId,'CreditLine').subscribe((res: any) => {
                        this.loader = false;
                        if (res) {
    ...
    onClickSendToLOS() {
        this.isSelectedSendToLOSPopup = true;
        this.getNbfcCompanyList();

    }
    getNbfcCompanyList() {
        this._leadService.GetNbfcCompaniesByProduct(this.customerData.productId).subscribe((res:any) => {
            console.log(res, 'res');
            if (res && res.length > 0) {
